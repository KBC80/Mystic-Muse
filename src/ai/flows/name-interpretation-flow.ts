...
const InterpretNameOutputSchema = z.object({
  ...
  detailedAnalysis: z.object({
    nameStructureAnalysis: z.object({
      hanjaStrokeCounts: z.array(z.object({ character: z.string(), strokes: z.number().int().optional() })).optional().describe("이름의 각 글자(한글, 또는 제공된 경우 한자)에 대한 획수와 그에 따른 음양입니다. 한자가 명시된 경우(예: '洪'), 해당 한자의 **정자(正字) 획수**를 사용합니다. 한글만 있는 음절의 경우, 일반적인 한글 획수 계산법을 적용합니다. 각 항목은 { character: \"글자\", strokes: 획수 } 형태로 제공되어야 합니다."),
...
2.  **이름 분석:**
    *   **한자 획수 및 음양 (hanjaStrokeCounts 및 yinYangHarmony):**
        *   먼저, 이름의 각 글자(한자 또는 한글)에 대한 획수를 계산하여 \`hanjaStrokeCounts\` 배열에 저장합니다. 한자가 명시된 경우(예: '洪'), 해당 한자의 **정자(正字) 획수**를 사용합니다. 한글만 있는 음절의 경우, 일반적인 한글 획수 계산법(예: ㄱ=1획, ㅏ=1획 등)을 적용합니다. 각 \`hanjaStrokeCounts\` 항목에는 { character: "글자", strokes: 획수 } 형태로 제공되어야 하며, 획수를 알 수 없는 경우 strokes 필드를 생략할 수 있습니다.
        *   다음으로, \`hanjaStrokeCounts\`의 각 글자 획수를 기반으로 음양을 판단합니다: **홀수 획수는 양(陽), 짝수 획수는 음(陰)**입니다.
        *   \`yinYangHarmony.nameYinYangComposition\` 필드에는 이름 각 글자의 음양을 순서대로 배열하여 문자열로 표시합니다 (예: "洪"(10획, 음), "吉"(6획, 음), "童"(12획, 음) 이면 "陰-陰-陰"으로, "李"(7획, 양), "明"(8획, 음), "博"(12획, 음)이면 "陽-陰-陰"으로 정확히 명시합니다). 이 음양 배열은 반드시 \`hanjaStrokeCounts\`에서 계산된 각 글자 획수 결과와 일치해야 합니다.
    ...
    *   **수리길흉 분석 (81수리 이론 기반 원형이정 4격):**
        *   **원격(元格, 초년운 0-20세):** (성이 한 글자일 경우) 이름 첫 글자 획수 + 이름 두번째 글자 획수. (성이 두 글자일 경우) 성씨 두번째 글자 획수 + 이름 첫 글자 획수. (이름이 외자일 경우) 이름 첫 글자 획수 + 1획. 산출된 수를 81수리표에 대입하여 길흉과 의미를 해석합니다. 해당 시기의 성격적 특징, 주요 운세 흐름(학업, 친구관계 등), 건강, 잠재력에 대해 **81수리 이론의 해당 번호 설명을 참고하여 구체적이고 심층적으로 설명**합니다. 
        *   **형격(亨格, 청년운 21-40세):** 성씨 첫 글자 획수 + 이름 첫 글자 획수. (성이 두 글자일 경우) 성씨 첫 글자 획수 + 성씨 두번째 글자 획수. 산출된 수를 81수리표에 대입하여 길흉과 의미를 해석합니다. 사회생활 시작, 직업, 결혼, 재물 형성, 대인관계 확장 등 청년기의 주요 과업과 운세를 중심으로 **81수리 이론의 해당 번호 설명을 참고하여 구체적이고 심층적으로 설명**합니다. 
        *   **이격(利格, 장년운 41-60세):** 성씨 첫 글자 획수 + 이름 마지막 글자 획수. (성이 두 글자일 경우) 성씨 첫 글자 획수 + 이름 마지막 글자 획수. (외자 이름일 경우) 성씨 획수 + 1획. 산출된 수를 81수리표에 대입하여 길흉과 의미를 해석합니다. 사회적 성취, 가정의 안정, 건강 변화, 자녀 관계 등 장년기의 운세 변화와 특징을 **81수리 이론의 해당 번호 설명을 참고하여 구체적이고 심층적으로 분석**합니다. 
        *   **정격(貞格, 말년운/총운 60세 이후):** 성씨와 이름의 모든 글자 획수의 총합. 산출된 수를 81수리표에 대입하여 길흉과 의미를 해석합니다. 인생 전체를 아우르는 총운이자 노년기의 건강, 안정, 자손과의 관계, 삶의 마무리 등을 **81수리 이론의 해당 번호 설명을 참고하여 종합적이고 심층적으로 조망**합니다. 
