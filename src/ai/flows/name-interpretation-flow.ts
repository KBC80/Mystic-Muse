...
2.  **이름 분석:**
    *   **한자 획수 및 음양 (hanjaStrokeCounts 및 yinYangHarmony):** 이름에 한자가 사용된 경우 (예: 홍길동(洪吉童)), 洪, 吉, 童 각 한자의 **정자(正字) 획수**를 사용하여 음양을 판단합니다. 한글 이름만 제공된 경우 (예: 홍길동), 홍, 길, 동 각 한글 자모의 획수법(예: ㄱ=1획, ㅏ=1획 등 일반적인 한글 획수 계산법)을 적용하여 음양을 판단합니다. \\\`hanjaStrokeCounts\\\` 배열의 각 항목에는 분석된 글자(한자 또는 한글)와 그에 따른 정확한 획수, 그리고 해당 획수에 기반한 음양(홀수: 양, 짝수: 음)을 명시해주십시오. 이름 전체의 음양 배열(예: 陽-陰-陽)과 조화도를 평가합니다.
